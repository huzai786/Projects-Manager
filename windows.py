...
from utils.utils import generate_project_showcase_layout, ProjectType
...
def config_window():
    return sg.Window('Please configure settings before further proceeding.',
                     [[sg.Text("Enter root drive letter (only write the letter)\t"), sg.InputText(key='PATH')],
                      [sg.Text('Enter folder name \t\t'), sg.InputText(key='FOLD')],
                      [sg.T("Enter token value and env variable to save, leave empty if already created:")],
                      [sg.T("Enter Github personal access token: \t\t"), sg.InputText(key="TOKEN")],
                      [sg.T("Enter environment variable name(default: github_token):\t"), sg.InputText(key="ENV")],
                      [sg.B("Continue"), sg.Cancel()]],
                     disable_close=True, modal=True, keep_on_top=True).read(close=True)
...
def main_window():
    ...
    window = sg.Window("Projects Manager", layout, size=(1380, 650))
    return window